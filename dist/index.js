...
(() => {
  ...
  function fadeBatch() {
    gsapWithCSS.set(".a--fade-b", { opacity: 0.1, y: 30 });
    ScrollTrigger2.batch(".a--fade-b", {
      onEnter: (batch) => gsapWithCSS.to(batch, { opacity: 1, y: 0, stagger: 0.1 }),
      onLeave: (batch) => gsapWithCSS.to(batch, { opacity: 0, y: 30 }),
      onEnterBack: (batch) => gsapWithCSS.to(batch, { opacity: 1, y: 0, stagger: 0 }),
      onLeaveBack: (batch) => gsapWithCSS.to(batch, { opacity: 0, y: 30 }),
      start: "top 85%",
      end: "bottom 15%",
      markers: false
    });
